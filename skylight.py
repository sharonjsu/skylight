...
# -*- coding: utf-8 -*-
"""
Created on Tue May  4 10:36:46 2021

@author: sharonjsu, code andequations adapted for python 3 from evgkanias: https://github.com/InsectRobotics/insectvision
"""
...
class Sky(Environment):
    ...
    def __call__(self, theta=None, phi=None, noise=0., eta=None, uniform_polariser=False):
        ...
        a_f = 0
        b_f = 1
        z = a_f+(b_f*theta_s*np.cos(theta)*(phi_s+np.pi)*np.cos(phi))
        f = (1+z)/(1-z)
    ...
    #     return s
def visualize_contrast(sky):
    import matplotlib.pyplot as plt

    plt.figure("spectral contrast", figsize=(4.5, 4.5))
    ax = plt.subplot(111, polar=True)
    ax.set_theta_zero_location("N")
    ax.set_theta_direction(-1)

    theta_s, phi_s = tilt(sky.theta_t, sky.phi_t, theta=sky.theta_s, phi=sky.phi_s)
    ax.scatter(sky.phi, sky.theta, s=20, c=sky.f, marker='.', cmap='viridis', vmin=-1, vmax=2)
...
if __name__ == "__main__":
    ...
    s = Sky(theta_s=np.pi/6) #30 deegrees
    # s = sky_from_observer()
    # s = Sky.from_type(11)
    # s.theta_s = np.pi/6
    y, p, a, f = s(t, p)
